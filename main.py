...
# Load product data from MongoDB or fallback to JSON
def load_products():
    try:
        db = get_mongodb_connection()
        if db is None:
            print("Failed to connect to MongoDB, loading from JSON fallback...")
            with open("jib_products_pages_1_to_5.json", "r", encoding="utf-8") as f:
                products = json.load(f)
            print(f"✅ Loaded {len(products)} products from JSON fallback")
            return products
        ...
        if not products:
            print("MongoDB returned no products, loading from JSON fallback...")
            with open("jib_products_pages_1_to_5.json", "r", encoding="utf-8") as f:
                products = json.load(f)
            print(f"✅ Loaded {len(products)} products from JSON fallback")
            return products
        ...
        print("Loading from JSON fallback...")
        with open("jib_products_pages_1_to_5.json", "r", encoding="utf-8") as f:
            products = json.load(f)
        print(f"✅ Loaded {len(products)} products from JSON fallback")
        return products
